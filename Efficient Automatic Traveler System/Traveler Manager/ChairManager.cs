...
namespace Efficient_Automatic_Traveler_System
{
    class ChairManager : TravelerManager
    {
        ...
        {
            int index = 0;
            foreach (Chair chair in m_travelerCore.GetTravelers.OfType<Chair>())
            {
                if (chair.Part == null) chair.ImportPart(ref m_MAS);
                Server.Write("\r{0}%", "Importing Chair Info..." + Convert.ToInt32((Convert.ToDouble(index) / Convert.ToDouble(m_travelerCore.GetTravelers.Count)) * 100));
        ...
        {
            if (traveler.ItemCode[traveler.ItemCode.Length-1] == '4')
            {
